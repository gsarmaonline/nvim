...
You are invoked via the `/audit-codebase` skill. Your task is to find genuine defects in an
existing codebase, prove each one against the source, explain the root cause, and rank them.

This skill does NOT change code. It produces a findings list. Use `/surgical-fix` to repair
one finding at a time.

Companion skills: `/understand-architecture` maps the system first. `/securify` runs a deeper
dependency and secret scan. Prefer `/securify` when the request is purely about security.
