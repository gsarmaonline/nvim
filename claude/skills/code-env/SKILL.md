...
If the project has both `go.mod` and a `frontend/` directory (or a Next.js `package.json`), activate **full-stack mode**. In this mode, `/code-env` does more than document — it also scaffolds Google OAuth and CORS code.
...
- Use the `/code-env` skill to scan and update `.env.example` whenever new environment variables are added
- Before creating PRs that add new env vars, run `/code-env` to update documentation
