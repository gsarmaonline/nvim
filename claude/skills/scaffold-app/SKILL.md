...
If the user passes arguments (e.g. `/scaffold-app myapp github.com/acme/myapp users "A simple task manager"`), parse them directly without asking.
...
Invoke the `/scaffold-go-backend` skill with:
- Module: `<module>`
- App name: `<appname>`
- Port: `<BACKEND_PORT>`
- Domain: `<domain>`
...
Invoke the `/scaffold-nextjs-frontend` skill with:
...
The `/scaffold-go-backend` skill creates a Makefile for the backend. Add root-level convenience targets that start both services together.
...
Invoke the `/code-ci` skill. It will:
...
Invoke the `/code-screenshots` skill to:
...
Invoke the `/code-security` skill to run a baseline security scan:
...
Skills invoked:
  /scaffold-go-backend     Go API, sqlc, Makefile, Dockerfile
  /scaffold-nextjs-frontend  Next.js app, API client, Dockerfile
  /code-env                .env.example with all vars documented
  /code-ci             .github/workflows/ci.yml
  /code-screenshots         frontend/screenshots/
  /code-security              baseline security scan
